...
<div id="contents" class="contents">
	<div class="container px-4 px-lg-5 mt-5">
		<div class="row">
			...
			<form action="${path1 }/payment/insertPayment.do" method="post" onsubmit="return salesCk(this)">
				<table class="table">
					<tbody>
						<tr>
							...
							<td>
								...
								<c:if test="${empty ccdoe }">
								<input type="hidden" name="ccode" id="ccode" value="0">
								</c:if>
						...
						<tr>
							<th><label for="cprice">가격</label></th>
							<td>${cfbean.cprice }</td>
						</tr>
						<tr>
							...
							<td>
								...
								<input type="number" name="amount" id="amount" min="1" max="${cfbean.amount }" value="1">
								</c:if>
								<c:if test="${!empty amount}">
								<input type="number" name="amount" id="amount" min="1" max="${cfbean.amount }" value="${camount }">
								</c:if>
							</td>
						</tr>
						<tr>
							<th><label for="money">결제할 금액</label></th>
							<td><input type="hidden" name="money" id="money">
							<span id="money_res">${cfbean.cprice }</span></td>
						</tr>
						<tr>
			...
			function payment() {
				...
				payPop = window.open("pay.do", "결제", "width=400, height=300");
			...
			$(document).ready(function(){
				$("#amount").on("keyup input change", function(){
					var gstock = $(this).val();
					var money = gstock*${cfbean.gprice};
					$("#money").val(money);
